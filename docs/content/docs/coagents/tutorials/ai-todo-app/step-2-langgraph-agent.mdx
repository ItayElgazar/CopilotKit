...
The LLM used in this agent is OpenAI, specifically the `gpt-4o` model. 

You can get an API key from the OpenAI console [here](https://platform.openai.com/api-keys).

### Google Places API Key
This agent uses the Google Places API to search for places. For this to work properly, you'll need to get an API key from the Google Cloud Console.

Luckily, getting an API key is quick and easy. Google has a guide which you can follow [here](https://developers.google.com/maps/documentation/places/web-service/get-api-key#creating-api-keys).
...
In the `examples/coagens-travel/agent/` directory, go ahead and create a file called `.env`.
...
- **Transitions**: Transitions are the arrows that connect nodes together. They define the logic for how the agent will move from one node to the next. They are defined in code as edges and conditional logic is handled with a `route` function.
...
![LangGraph Studio New Message](/images/coagents/tutorials/ai-travel-app/lgs-new-message.png)
...
agent calls the `search_node` and, once it has received a response, will request to add a new trip based on its findings to the state
via the `trips_node`.

One very important thing to note is the "continue" button that appears as a result of the interrupt. This will be the mechanism by which
we implement human-in-the-loop later in this tutorial.
